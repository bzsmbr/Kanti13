...
export const updateBook = (title, author) => db.prepare('UPDATE books SET title = ?, author = ? WHERE id = ?').run(title, author, id);
...
if (!books.length) {
    ...
    saveBook("The are of war", "Sun Tzu");
    saveBook("Mein kampf", "Adolf Hitler");
}